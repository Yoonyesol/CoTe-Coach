...
interface LogFormProps {
    ...
    onCancel?: () => void;
    submitLabel?: string;
    mode?: 'LOG' | 'PROBLEM' | 'ALL';
}

const LogForm: React.FC<LogFormProps> = ({ initialValues, onSubmit, onCancel, submitLabel = '제출하기', mode = 'LOG' }) => {
    const [problemTitle, setProblemTitle] = useState(initialValues?.problemTitle || '');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {(mode === 'PROBLEM' || mode === 'ALL') && (
                <div className="space-y-4 p-3 sm:p-5 bg-base-50/50 rounded-2xl border border-base-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-base-400 uppercase tracking-widest px-1">문제 링크 (URL)</label>
                            <input
                                type="text"
                                value={url}
            ...
            {(mode === 'LOG' || mode === 'ALL') && (
                <>
                    ...
                    {/* Language Selection & Time Input Grid */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Language Selection */}
                        <div className="space-y-3">
                            <label className="text-xs font-black text-base-700 uppercase tracking-widest flex items-center gap-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {languages.map((lang) => (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            language === lang
                                                ? "bg-base-900 border-base-900 text-white shadow-md"
                                                : "bg-white border-base-100 text-base-400 hover:border-base-200"
                                        )}
                                    >
                                        {lang}
                                ...
                                <input
                                    ...
                                    className={clsx(
                                        "px-3 py-1.5 rounded-xl text-[10px] font-black transition-all border-2 outline-none w-20",

...
const DailyPlanner: React.FC<DailyPlannerProps> = ({ solvedCount, goalCount, daysRemaining, streak }) => {
    ...
    return (
        <div className="glass-card p-6 md:p-8 border-none bg-gradient-to-br from-white/80 to-misty-light/30 shadow-xl relative overflow-hidden group">
            {/* Background Accent */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row lg:flex-row gap-6 md:gap-8 items-center">
                ...
                <div className="relative w-28 h-28 md:w-32 md:h-32 shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-xl md:text-2xl font-black text-base-900 leading-none">{percentage}%</span>
                        <span className="text-[9px] md:text-[10px] font-bold text-base-400 uppercase mt-1">완료</span>
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-4 text-center md:text-left">
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-3 md:gap-4">
                        ...
                        <div className="flex items-center gap-2 px-3 md:px-4 py-2 bg-white/60 rounded-xl border border-white shadow-sm font-sans">
                            <Trophy className="w-3.5 h-3.5 text-base-400" />
                            <span className="text-[10px] font-black text-base-600 uppercase">연속 달성</span>
                            <span className="text-xs md:text-sm font-black text-sage-dark">{streak}일</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="flex justify-center md:justify-start gap-3 min-w-max px-2">
                    {Array.from({ length: goalCount }).map((_, i) => (
                        <div key={i} className="flex flex-col items-center gap-1">
                            {i < solvedCount ? (
                                <CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 text-sage animate-in zoom-in duration-300" />
                            ) : (
                                <Circle className="w-5 h-5 md:w-6 md:h-6 text-base-100" />
                            )}
                            <span className={clsx(

...
const DeleteAccountModal: React.FC<DeleteAccountModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleProceedToFarewell = async () => {
        if (!password) {
            showAlert("비밀번호 확인", "본인 확인을 위해 비밀번호를 입력해 주세요.");
            return;
        }

        if (selectedReason === "직접 입력" && !customReason.trim()) {
            showAlert("사유 입력", "탈퇴 사유를 입력해 주세요.");
            return;
        }
        ...
        try {
            ...
            console.error(error);
            showAlert("오류", "비밀번호 확인 중 문제가 발생했습니다.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        setIsLoading(true);
        try {
            const finalReason = selectedReason === "직접 입력" ? customReason : selectedReason;
            await deleteAccount(finalReason);
            showAlert("탈퇴 완료", "그동안 이용해 주셔서 감사합니다.");
            onClose();
            // Redirect or logout logic handled by store
        } catch (error) {
            console.error(error);
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4">
                    ...
                    <motion.div
                        ...
                        animate="visible"
                        exit="exit"
                        className="glass-card w-full max-w-md bg-white border-none shadow-2xl relative z-10 overflow-hidden flex flex-col sm:rounded-2xl rounded-t-2xl rounded-b-none sm:rounded-b-2xl max-h-[80vh] sm:max-h-auto"
                    >
                        {/* Header */}
